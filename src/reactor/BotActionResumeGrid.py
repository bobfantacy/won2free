...
class BotActionResumeGrid(AbstractBotAction):
  ...
  async def _execute(self, event_body):
    ...
    if(len(data) < 2):
      self.send_message("Invalid command, please use: ResumeGrid <account_name> <grid_id>")
      return
    
    grid = session.query(OrderGridStrategy).filter(OrderGridStrategy.account_id == config.get_cur_account().id,
                                                    OrderGridStrategy.id == data[0]).first()
    if(grid==None):
      self.send_message(f"Strategy {data[0]} NOT Exist!")
      return
    if(grid.status != 'executing'):
      self.send_message(f"Strategy {grid.id} is NOT executing!")
      return
    ...
    if grid.price_change_type == 'difference':
      ...
      if buy_order is None:
        try:
          ...
          self.send_message(f"Place Buy Order fail: {e}")
      else:
        try:
          await self.update_order(buy_order.id, buy_quantity, lower_price, grid.id, oper_count)
        except Exception as e:
          self.send_message(f"Update Buy Order fail: {e}")
      
      if sell_order is None:
        try:
          await self.sell(grid.symbol, sell_quantity, upper_price, grid.id, oper_count)
        except Exception as e:
          self.send_message(f"Place Sell Order fail: {e}")
      else:
        try:
          await self.update_order(sell_order.id, -sell_quantity, upper_price, grid.id, oper_count)
        except Exception as e:
          self.send_message(f"Update Sell Order fail: {e}")
      
      grid.oper_count = oper_count + 1
      grid.status ='executing'
      self.send_message(f"Rusume Grid Strategy {grid.id}: {grid.symbol} {grid.latest_base_price:5.0f} {grid.price_change_type} {+grid.every_rise_by:4.0f} {-grid.every_fall_by:4.0f} {grid.order_quantity_per_trade:.5f}")
